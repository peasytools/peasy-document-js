...
/** PeasyDocument API client. Zero dependencies — uses native fetch. */
export class PeasyDocument {
  private baseUrl: string;

  constructor(baseUrl = "https://peasyformats.com") {
  ...
  private async get<T>(path: string, params?: Record<string, string | number | undefined> | object): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`PeasyDocument API error: HTTP ${res.status} — ${body}`);
    }
